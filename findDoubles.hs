...
openFile' file = openFile file ReadMode `catch` reOpen
  where
    reOpen :: IOException -> IO Handle
    reOpen _ = performGC >> openFile' file
...
run path outputFile = do
  ...
  putStrLn "Done!"
    where
      showMatches :: [FileInfo] -> String
      showMatches aGroup =
        "MD5 hash: " ++ (show . fiHash . head) aGroup ++
        "\nFiles:\n" ++
        intercalate "\n" (map fiFile aGroup) ++ "\n"
...
main = do
  args <- getArgs
  case args of
    (path:outputFile:[]) -> run path outputFile
